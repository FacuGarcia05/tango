...
export class AuthController {
  ...
  @Post("register")
  @ApiOperation({ summary: "Registrar usuario (requiere verificacion por email)" })
  @ApiBody({
    schema: {
      properties: {
  ...
  @ApiOperation({ summary: "Login con email y password (requiere email verificado)" })
  ...
  async resendVerification(@Body() body: ResendVerificationDto) {
    ...
    return { message: "Si el email existe, reenviamos el enlace de verificacion." };
  }
  ...
  async verifyEmail(@Req() req: Request, @Res() res: Response) {
    ...
    if (!token) {
      return res.status(HttpStatus.BAD_REQUEST).json({ message: "Token requerido" });
    }

    await this.service.verifyEmail(token);
    const redirectUrl = new URL(this.config.get<string>("FRONTEND_BASE_URL") ?? "http://localhost:3000");
  ...
  async googleCallback(@Req() req: Request & { user?: { access_token: string } }, @Res() res: Response) {
