...
export class AuthService {
  ...
  async handleGoogleLogin(profile: Profile) {
    ...
    const displayName = profile.displayName || `${profile.name?.givenName ?? ""} ${profile.name?.familyName ?? ""}`.trim() || email;
  ...
  private async sendEmailVerification(userId: string, email: string) {
    ...
    const token = await this.jwt.signAsync({ sub: userId, email }, { secret, expiresIn: "24h" });
