...
describe('UsersService', () => {
  ...
  beforeEach(() => {
    ...
    (prismaMock.$transaction as jest.Mock).mockImplementation(
      (operations: any[]) => Promise.all(operations),
    );
  ...
  describe('follow', () => {
    ...
    it('should throw if relationship already exists', async () => {
      ...
      await expect(service.follow('user-1', 'target')).rejects.toThrow(
        ConflictException,
      );
    });
    ...
    it('should create follow and return updated stats', async () => {
      ...
      (prismaMock.follows.findUnique as jest.Mock).mockResolvedValue({
        follower_id: 'user-1',
      });
  ...
  describe('unfollow', () => {
    it('should throw if link does not exist', async () => {
      ...
      await expect(service.unfollow('user-1', 'target')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
    ...
    it('maps follow stats and following state', async () => {
      ...
      (prismaMock.follows.groupBy as jest.Mock)
        .mockResolvedValueOnce([
          { followee_id: 'target', _count: { followee_id: 12 } },
        ])
        .mockResolvedValueOnce([
          { follower_id: 'target', _count: { follower_id: 4 } },
        ]);
      (prismaMock.follows.findMany as jest.Mock).mockResolvedValue([
        { followee_id: 'target' },
      ]);
